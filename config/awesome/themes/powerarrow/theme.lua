...
local function pl(widget, bgcolor, padding)
    _padding = padding == nil and 8 or padding
    return wibox.container.background(wibox.container.margin(widget, _padding, _padding), bgcolor, theme.powerline_rl)
end

--[[ Redshift
-- local on = "#CB755B"
local redshift = wibox.widget.imagebox(theme.widget_hdd)
-- local redshift = wibox.widget.textbox()
local redshift_pl = pl(wibox.widget { nil, redshift, layout = wibox.layout.align.horizontal }, "#CB755B"),
lain.widget.contrib.redshift:attach(
    redshift_pl,
    function (active)
        if active then
            --redshift_pl:set_bg("#CB755B")
        else
            --redshift_pl:set_bg("#777E76")
        end
    end
)
--]]
